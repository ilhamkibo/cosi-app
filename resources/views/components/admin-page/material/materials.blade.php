<x-layouts.admin.admin-app>
    ...
    <div class="border-2 border-dashed rounded-lg border-gray-300 dark:border-gray-600 mb-4">
        ...
        <div class="p-2 flex flex-col lg:flex-row gap-4 justify-between items-center">
            ...
            <div>
                <form class="flex items-center" action="{{ route('admin.materials.index') }}" method="GET">
                    @csrf
        ...
        <div class="relative overflow-x-auto px-2 ">
            <table class="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @forelse ($materials as $ind => $material)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <th scope="row"
                                class="px-6 py-4 {{ $ind === last($materials) ? 'rounded-bl-md' : '' }} font-medium text-gray-900 dark:text-white">
                                {{ $ind + 1 }}
                            </th>
                            <th scope="row"
                            ...
                            <td class="px-6 py-4">
                                IDR. {{ number_format($material->price) }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $material->unit }}
                            </td>
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::parse($material->updated_at)->addHours(7) ?? $material->created_at }}
                            </td>
                            <td class="px-6 py-4 {{ $ind === last($materials) ? 'rounded-br-md' : '' }} min-w-[200px]">
                                ...
                                <button type="submit"
                                    ...
                                    data-modal-target="modal-trash-{{ $material->id }}"
                                    data-modal-toggle="modal-trash-{{ $material->id }}">trash</button>
                            </td>
                        </tr>


                        <div id="modal-trash-{{ $material->id }}" tabindex="-1"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <button type="button"
                                        class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                        data-modal-hide="modal-trash-{{ $material->id }}">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you
                                            sure you want to send this material to trash?</h3>
                                        <form action="{{ route('admin.materials.destroy', $material->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button data-modal-hide="modal-trash-{{ $material->id }}" type="submit"
                                        ...
                                        <button data-modal-hide="modal-trash-{{ $material->id }}" type="button"
            ...
            <table class="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                    <tr class="bg-yellow-300 dark:bg-gray-700 dark:text-gray-400 ">
                        <th colspan="9" class="px-6 py-3 rounded-t-md text-xl">Materials Trashed</th>
                    </tr>
                    <tr>
                        <th scope="col" class="px-6 py-3">
                            No.
                ...
                <tbody>
                    @forelse ($trashedMaterials as $ind => $material)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <th scope="row"
                                class="px-6 py-4 {{ $ind === last($trashedMaterials) ? 'rounded-bl-md' : '' }} font-medium text-gray-900 dark:text-white">
                                {{ $ind + 1 }}
                            </th>
                            <th scope="row"
                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            ...
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::parse($material->deleted_at)->addHours(7) ?? $material->created_at }}
                            </td>
                            <td
                                class="px-6 py-4 {{ $ind === last($materials) ? 'rounded-br-md' : '' }} min-w-[200px]">
                                <button type="submit"
                                    class="bg-green-500 hover:bg-green-700 text-white py-2 px-2 rounded"
                                    data-modal-target="modal-restore-{{ $material->id }}"
                                    data-modal-toggle="modal-restore-{{ $material->id }}">Restore</button>
                                <button type="submit"
                                    class="bg-red-500 hover:bg-red-700 text-white py-2 px-2 rounded"
                                    data-modal-target="modal-delete-{{ $material->id }}"
                                    data-modal-toggle="modal-delete-{{ $material->id }}">Delete</button>
                            </td>
                        </tr>


                        <div id="modal-restore-{{ $material->id }}" tabindex="-1"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <button type="button"
                                        class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                        data-modal-hide="modal-restore-{{ $material->id }}">
                                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                            fill="none" viewBox="0 0 14 14">
                                            <path stroke="currentColor" stroke-linecap="round"
                                                stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you
                                            sure you want to restore this material?</h3>
                                        <form action="{{ route('admin.materials.restore', $material->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <button data-modal-hide="modal-restore-{{ $material->id }}"
                                                type="submit"
                                        ...
                                        <button data-modal-hide="modal-restore-{{ $material->id }}" type="button"
                                            class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">No,
                                            cancel</button>
                                    </div>
                                </div>
                        ...
                        <div id="modal-delete-{{ $material->id }}" tabindex="-1"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <button type="button"
                                        class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                        data-modal-hide="modal-delete-{{ $material->id }}">
                                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                            fill="none" viewBox="0 0 14 14">
                                            <path stroke="currentColor" stroke-linecap="round"
                                                stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you
                                            sure you want to <span class="text-red-600">delete</span> this material
                                            <span class="text-red-600">permanently</span>?
                                        </h3>
                                        <form action="{{ route('admin.materials.permanentlyDelete', $material->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button data-modal-hide="modal-delete-{{ $material->id }}" type="submit"
                                                class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                                Yes, I'm sure
                                            </button>
                                        </form>
                                        <button data-modal-hide="modal-delete-{{ $material->id }}" type="button"
                                            class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">No,
                                            cancel</button>
                                    </div>
                                </div>
